...
from .gnss_xml import generate_great_xml
...
def _run_great_app(bindir, app, config, label, str_args="", xmldir=None, newxml=True, stop=True, **kwargs):
    ...
    if newxml:
        if os.path.isfile(f_xml):
            os.remove(f_xml)
        generate_great_xml(config, app, f_xml, **kwargs)
    else:
...
def _run_great_app_multithreading(bindir, app, config, label, str_args="", xmldir=None, nthread=8, stop=True, **kwargs):
    ...
    grt_app = _executable_app(bindir, app)
    stas_all = config.stalist()
    leos_all = config.leolist()
    stas, leos = gt.split_receivers(config, nthread)
    #child_configs = gt.split_config_by_receivers(config, nthread)
    #nthread = min(nthread, len(child_configs))
    nthread = min(nthread, len(stas)+len(leos))
    thread_list = []
    if xmldir:
        if not os.path.isdir(xmldir):
            os.makedirs(xmldir)
    ...
    for i in range(nthread):
        ...
        if i >= len(stas):
            config.update_stalist([])
            config.update_leolist(leos[i-len(stas)])
        else:
            config.update_stalist(stas[i])
            config.update_leolist([]) 
        generate_great_xml(config, app, f_xml, ithread=i+1, **kwargs)
        grt_cmd = f"{grt_app} -x {f_xml} {str_args} > {f_out} 2>&1"
        new_thread = Thread(target=_run_cmd, args=(grt_cmd, stop))
        thread_list.append(new_thread)
        new_thread.start()

    config.update_stalist(stas_all)
    config.update_leolist(leos_all)
    #for i in range(nthread):
        #if xmldir:
            #f_xml = os.path.join(xmldir, f"{label}{i + 1:0>2d}.xml")
        #else:
        ...
        #f_out = os.path.join('tmp', f"{label}{i + 1:0>2d}.log")
        #generate_great_xml(child_configs[i], app, f_xml, ithread=i + 1, **kwargs)
        #grt_cmd = f"{grt_app} -x {f_xml} {str_args} > {f_out} 2>&1"
        #new_thread = Thread(target=_run_cmd, args=(grt_cmd, stop))
        #thread_list.append(new_thread)
        #new_thread.start()
...
def _run_cmd(cmd, stop=True):
    ...
    try:
        ...
        return True
    except subprocess.CalledProcessError as dummy_e:
        if stop:
